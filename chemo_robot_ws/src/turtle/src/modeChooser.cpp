...
int main(int argc, char **argv)
{
  ...
  while (ros::ok())
  {
    ...
    while (send_Find_Trace_Bool == true){
      ...
      double tmp2;
      tmp2 = checkRBG();
      if (tmp2 < 500 && counterTimer > 1000){ //Make 700 for cyclic turtle navigation at 10,10,1,1,1
...
void send_Source_Found_Command(){
  std::cout<<"Found Source!!!!!!!!!!!!!!!!!!!!!!!!!\n";
  std::cout<<"YAY~!\n";
}